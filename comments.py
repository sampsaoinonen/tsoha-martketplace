...
def delete_ad_comment(user_id, ad_comment_id):    
    if session["admin"]:        
        result = db.session.execute("DELETE FROM ad_comments WHERE id=:ad_comment_id", {"ad_comment_id":ad_comment_id})
        db.session.commit()
    else:
        result = db.session.execute("DELETE FROM ad_comments WHERE id=:ad_comment_id AND user_id=:user_id",{"user_id":user_id, "ad_comment_id":ad_comment_id})
        db.session.commit()    
