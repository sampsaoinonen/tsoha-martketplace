...
def get_ads():
    sql = '''SELECT A.id, A.title, A.description, A.phone, A.email, A.location, A.price, A.sent_at, 
    U.username, C.cat_name FROM ads A, categories C, users U WHERE C.id=A.cat_id AND U.id=A.user_id
...
def add_ad(title, description, phone, email, location, price, user_id, cat_id):    
    ...
    VALUES (:title, :description, :phone, :email, :location, :price, NOW(), :user_id, :cat_id) RETURNING id'''
