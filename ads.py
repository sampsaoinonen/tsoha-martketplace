...
def get_ads():
    ...
    U.username, C.cat_name FROM ads A, categories C, users U, adtypes T WHERE C.id=A.cat_id AND U.id=A.user_id
    AND T.id=A.type_id AND A.sent_at > current_date - A.expires GROUP BY A.id, C.cat_name, U.username 
